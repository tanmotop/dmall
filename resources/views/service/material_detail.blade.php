...
<div class="zc-body">
    <div class="body-top">
        <div style="margin-top: 5px;margin-left: 10px;" class="top-left">
         ...
         <div class="tdcymd-top-center">
            ...
            <input type="text" class="search-value" value="{{ $keyword }}" placeholder="培训资料名称">
        </div>
   ...
   <div id="tra_list">
        @forelse($list as $item)
            <div class="sp-list-bottom">
                <div class="down">
                    <div class="down-left"><img src="/assets/img/{{ $item->file_ext }}.png"></div>
                    <div class="down-mid">
                        <div style="margin-top: 6px;"><p>{{ $item->title }}</p></div>
                        <div style="margin-top: 10px;">
                            <p style="color: orange">有附件</p>
                        </div>
                        <div style="margin-top: 10px;">
                            <p style="font-size: 4px;color: #CFCFD1;">{{ $item->created_at }}</p>
                        </div>
                    </div>
                    <div class="down-right">
                        <button onclick="download('{{ $item->attach }}')" type="button" class="sx-btn">点击下载</button>
                    </div>
                </div>
            </div>
        @empty
   ...
   @if ($list->currentPage() != $list->lastPage())
        <div align="center" onclick="getMoreMaterials()" class="sp-list-bottom tips"><span>上拉/点击获取更多邀请码</span></div>
    @endif
</div>
<br/><br/>
@endsection
...
<script>
    ...
    function getMoreMaterials()
    {
        ...
        $.get('{{ route('service_material_detail') }}', {
            type_id: '{{ $type->id }}',
            page: currentPage + 1,
            dataType: 'json',
            keyword: '{{ $keyword }}',
            ...
            for (var i in data) {
                ...
                html += ""
                    ...
                    + '        </div>'
                    + '        <div class="down-right">'
                    + '            <button type="button" onclick="download('+item.attach+')" class="sx-btn">点击下载</button>'
                    + '        </div>'
                    + '    </div>'
                    + '</div>';
            }
    ...
    $('.search-btn').on('click', function() {
        ...
        window.location = '{{ route('service_material_detail') }}?type_id={{$type->id}}&keyword=' + val
    })
